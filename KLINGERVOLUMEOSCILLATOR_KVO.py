...
def calculate_kvo(df, short_period=34, long_period=55, signal_period=13):
    ...
    Parameters:
    df (pd.DataFrame): The DataFrame containing stock data with 'close', 'high', 'low', and 'volume' columns.
    short_period (int): The short period for the KVO calculation.
    long_period (int): The long period for the KVO calculation.
    signal_period (int): The signal line period for the KVO calculation.
    
    Returns:
    pd.Series: A Series representing the KVO and signal line values.
    """
    # Calculate typical price
    df['typical_price'] = (df['high'] + df['low'] + df['close']) / 3
    
    # Calculate the price direction (trend)
    df['price_trend'] = df['typical_price'].diff().apply(lambda x: 1 if x > 0 else (-1 if x < 0 else 0))
    
    # Calculate volume force
    df['volume_force'] = df['volume'] * df['price_trend'] * abs(df['high'] - df['low'])
    
    # Calculate short and long EMAs of volume force
    df['short_kvo'] = df['volume_force'].ewm(span=short_period, adjust=False).mean()
    df['long_kvo'] = df['volume_force'].ewm(span=long_period, adjust=False).mean()
    
    # Calculate the KVO
    df['KVO'] = df['short_kvo'] - df['long_kvo']
    
    # Calculate the signal line (EMA of KVO)
    df['signal_line'] = df['KVO'].ewm(span=signal_period, adjust=False).mean()
    
    return df[['KVO', 'signal_line']]

# Function to fetch historical price data and calculate KVO
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates KVO.
    ...
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data, and KVO.
    """
    try:
        ...
        # Calculate KVO
        df[['KVO', 'signal_line']] = calculate_kvo(df)

        logging.info(f"Historical data fetched with KVO calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with KVO and Signal Line
    print(combined_df[['symbol', 'close', 'KVO', 'signal_line']].head())  # Show key columns

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_KVO.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'KVO', 'signal_line']].head())  # Show the first few rows with KVO and Signal Line
