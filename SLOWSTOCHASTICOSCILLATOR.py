...
# Function to calculate the Slow Stochastic Oscillator (%K and %D)
def calculate_slow_stochastic_oscillator(df, k_window=14, d_window=3, slow_d_window=3):
    """
    Calculate the Slow Stochastic Oscillator (%K and %D).
    
    Parameters:
    df (pd.DataFrame): DataFrame with historical data (with 'high', 'low', 'close').
    k_window (int): Window size for %K calculation (default 14).
    d_window (int): Window size for %D calculation (default 3 for fast %D).
    slow_d_window (int): Window size for Slow %D calculation (default 3).
    
    Returns:
    pd.DataFrame: DataFrame with Slow %K and Slow %D columns.
    """
    df = df.copy()
    ...
    df['%K_fast'] = 100 * ((df['close'] - df['lowest_low']) / (df['highest_high'] - df['lowest_low']))

    # Calculate Fast %D (SMA of Fast %K)
    df['%D_fast'] = df['%K_fast'].rolling(window=d_window).mean()

    # Slow %K is the Fast %D
    df['%K_slow'] = df['%D_fast']

    # Slow %D (SMA of Slow %K)
    df['%D_slow'] = df['%K_slow'].rolling(window=slow_d_window).mean()

    return df[['%K_slow', '%D_slow']]
...
if __name__ == "__main__":
    ...
    for share in shares:
        ...
        # Calculate the Slow Stochastic Oscillator for each stock
        stoch_df = calculate_slow_stochastic_oscillator(df)
    ...
    # Display the combined DataFrame with Slow Stochastic Oscillator (%K, %D)
    print("Combined DataFrame with Slow Stochastic Oscillator (combined_df):")
    print(combined_df[['symbol', 'close', '%K_slow', '%D_slow']].head())  # Show the first few rows with %K_slow and %D_slow

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_slow_stochastic.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', '%K_slow', '%D_slow']].head())  # Show the first few rows of individual stock data with %K_slow and %D_slow
