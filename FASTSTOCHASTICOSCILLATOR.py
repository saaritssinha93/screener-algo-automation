...
# Function to calculate the Fast Stochastic Oscillator (%K and %D)
...
if __name__ == "__main__":
    ...
    for share in shares:
        ...
        stoch_df = calculate_stochastic_oscillator(df)
    ...
    # Display the combined DataFrame with Stochastic Oscillator (%K, %D)
    print("Combined DataFrame with Stochastic Oscillator (combined_df):")
    print(combined_df[['symbol', 'close', '%K', '%D']].head())  # Show the first few rows with %K and %D

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_stochastic.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', '%K', '%D']].head())  # Show the first few rows of individual stock data with %K and %D
