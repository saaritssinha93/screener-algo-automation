...
def calculate_obv(df):
    ...
    for i in range(1, len(df)):
        if df['close'][i] > df['close'][i-1]:
            ...
            df['OBV'][i] = df['OBV'][i-1]  # No change in OBV if close price is the same
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with OBV
    print("Combined DataFrame with OBV (combined_df):")
    print(combined_df[['symbol', 'close', 'OBV']].head())  # Show the first few rows with OBV

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_obv.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'OBV']].head())  # Show the first few rows of individual stock data with OBV
