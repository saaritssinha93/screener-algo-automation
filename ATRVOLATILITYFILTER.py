...
# Function to apply ATR as a filter and for position sizing
def atr_volatility_filter(df, atr_threshold=1.5):
    """
    Use ATR as a volatility filter and for position sizing.
    
    Parameters:
    df (pd.DataFrame): The DataFrame containing stock data with 'ATR' and 'close' columns.
    atr_threshold (float): The threshold for ATR to be considered high volatility (default 1.5% of the stock's price).
    
    Returns:
    pd.DataFrame: A DataFrame with filtered trades based on ATR and position sizing.
    """
    # ATR filter for volatility breakout: Enter trades only when ATR > atr_threshold * close price
    df['atr_filter'] = df['ATR'] > (atr_threshold / 100) * df['close']
    
    # Position sizing based on ATR: Higher ATR values reduce position size
    # Position size = 1 / ATR (normalized to stock's price) for simplicity
    df['position_size'] = 1 / df['ATR']
    
    # Normalize position size (scaling down larger positions for high volatility)
    df['position_size'] = df['position_size'] / df['position_size'].max()
    
    return df

# Function to fetch historical price data and calculate ATR as filter for volatility breakout and position sizing
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day', atr_window=14, atr_threshold=1.5):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and applies ATR-based filters.
    ...
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data, ATR, volatility breakout, and position sizing.
    """
    try:
        ...
        # Apply ATR as a filter and for position sizing
        df = atr_volatility_filter(df, atr_threshold)

        logging.info(f"Historical data fetched with ATR filters applied for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with ATR-based volatility breakout filter and position sizing
    print(combined_df[['symbol', 'close', 'ATR', 'atr_filter', 'position_size']].head())  # Show key columns

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_ATR_filter.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'ATR', 'atr_filter', 'position_size']].head())  # Show the first few rows with ATR-based filters
