...
# Function to fetch historical price data and calculate Accumulation/Distribution (A/D) Line
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates the Accumulation/Distribution (A/D) Line.
    ...
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data and A/D Line.
    """
    try:
        ...
        # Initialize A/D Line
        df['AD_Line'] = df['money_flow_volume'].cumsum()  # Cumulative sum of money flow volume
        ...
        logging.info(f"Historical data fetched and A/D Line calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with A/D Line
    print(combined_df[['symbol', 'close', 'AD_Line']].head())  # Show the first few rows with A/D Line

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_ad_line.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'AD_Line']].head())  # Show the first few rows of individual stock data with A/D Line
