...
def calculate_cmo(df, period=14):
    ...
    period (int): The period over which to calculate the CMO (default 14).
    
    Returns:
    pd.Series: A Series representing the CMO values.
    """
    df['change'] = df['close'].diff()

    df['gains'] = df['change'].apply(lambda x: x if x > 0 else 0)
    df['losses'] = df['change'].apply(lambda x: -x if x < 0 else 0)

    df['sum_gains'] = df['gains'].rolling(window=period).sum()
    df['sum_losses'] = df['losses'].rolling(window=period).sum()

    df['CMO'] = 100 * (df['sum_gains'] - df['sum_losses']) / (df['sum_gains'] + df['sum_losses'])

    return df['CMO']

# Function to fetch historical price data and calculate CMO
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates CMO.
    ...
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data, and CMO.
    """
    try:
        ...
        # Handle NaN values in the CMO calculation using forward fill and backward fill
        df.ffill(inplace=True)
        df.bfill(inplace=True)

        logging.info(f"Historical data fetched with CMO calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with CMO
    print(combined_df[['symbol', 'close', 'CMO']].head())  # Show key columns

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_CMO.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'CMO']].head())  # Show the first few rows with CMO
