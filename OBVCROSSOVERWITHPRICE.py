...
# Function to detect OBV crossover with price
def detect_obv_crossover(df):
    """
    Detect OBV crossover with price (bullish or bearish crossover).
    
    Parameters:
    df (pd.DataFrame): DataFrame with historical data (must include 'close' and 'OBV').
    
    Returns:
    pd.Series: Signal for crossover ('bullish_crossover', 'bearish_crossover', or 'no_crossover').
    """
    df['Crossover'] = 'no_crossover'  # Initialize with no crossover

    for i in range(1, len(df)):
        # Bullish crossover: OBV crosses above the price
        if df.loc[i-1, 'OBV'] < df.loc[i-1, 'close'] and df.loc[i, 'OBV'] > df.loc[i, 'close']:
            df.loc[i, 'Crossover'] = 'bullish_crossover'
        
        # Bearish crossover: OBV crosses below the price
        elif df.loc[i-1, 'OBV'] > df.loc[i-1, 'close'] and df.loc[i, 'OBV'] < df.loc[i, 'close']:
            df.loc[i, 'Crossover'] = 'bearish_crossover'
...
if __name__ == "__main__":
    ...
    for share in shares:
        ...
        # Detect OBV crossover with price for each stock
        df['Crossover'] = detect_obv_crossover(df)
    ...
    # Display the combined DataFrame with OBV and Crossover
    print("Combined DataFrame with OBV and Crossover (combined_df):")
    print(combined_df[['symbol', 'close', 'OBV', 'Crossover']].head())  # Show the first few rows with OBV and crossover signals

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_obv_crossover.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'OBV', 'Crossover']].head())  # Show the first few rows of individual stock data with OBV and crossover signals
