...
# Function to calculate ATR with RSI combined strategy
def calculate_atr_rsi_strategy(df, atr_window=14, rsi_window=14):
    """
    Combine ATR and RSI strategy. ATR measures volatility, and RSI identifies momentum and overbought/oversold conditions.
    ...
    Returns:
    pd.DataFrame: A DataFrame with ATR and RSI combined.
    """
    # Calculate ATR
    df['ATR'] = calculate_atr(df, window=atr_window)
    
    # Calculate RSI
    df['RSI'] = calculate_rsi(df, n=rsi_window)

    return df

# Function to fetch historical price data and calculate ATR with RSI strategy
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates ATR with RSI strategy.
    ...
    Returns:
    pd.DataFrame: A DataFrame with ATR and RSI values.
    """
    try:
        ...
        # Calculate ATR with RSI
        df = calculate_atr_rsi_strategy(df)
        ...
        logging.info(f"Historical data fetched and ATR with RSI strategy calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with ATR and RSI
    print(combined_df[['symbol', 'close', 'ATR', 'RSI']].head())  # Show the first few rows with ATR and RSI

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_atr_rsi.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'ATR', 'RSI']].head())  # Show the first few rows of individual stock data with ATR and RSI
