...
def calculate_parabolic_sar(df, af_start=0.02, af_step=0.02, af_max=0.2):
    """
    Calculate the Parabolic SAR (Stop and Reverse) for a DataFrame.

    Parameters:
    df (pd.DataFrame): DataFrame with historical data (must include 'high', 'low', 'close').
    af_start (float): Starting value of acceleration factor (default 0.02).
    af_step (float): Increment of acceleration factor (default 0.02).
    af_max (float): Maximum value of acceleration factor (default 0.2).
    ...
    long_position = True
    af = af_start
    ep = df['low'].iloc[0]  # Extreme price
    sar = df['high'].iloc[0]  # Initial SAR
    ...
    for i in range(1, len(df)):
        ...
        if long_position:
            sar = sar + af * (ep - sar)
            sar = min(sar, df['low'].iloc[i-1], df['low'].iloc[i])
            ...
            sar = sar + af * (ep - sar)
            sar = max(sar, df['high'].iloc[i-1], df['high'].iloc[i])
            if df['low'].iloc[i] < ep:
                ep = df['low'].iloc[i]
                af = min(af + af_step, af_max)
...
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    ...
    try:
        ...
        logging.info(f"Historical data fetched and Parabolic SAR calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with Parabolic SAR
    print(combined_df[['symbol', 'close', 'Parabolic_SAR']].head())  # Show key columns

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_Parabolic_SAR.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'Parabolic_SAR']].head())  # Show the first few rows of individual stock data
