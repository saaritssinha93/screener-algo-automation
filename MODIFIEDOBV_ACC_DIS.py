...
# Function to fetch historical price data and calculate A/D OBV
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates Modified OBV (Accumulation/Distribution).
    ...
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data and Accumulation/Distribution OBV.
    """
    try:
...
# Function to calculate Accumulation/Distribution (A/D) OBV
def calculate_ad_obv(df):
    """
    Calculate the Modified OBV (Accumulation/Distribution) indicator.
    
    Parameters:
    df (pd.DataFrame): DataFrame with historical data (must include 'close', 'high', 'low', and 'volume').
    
    Returns:
    pd.Series: A/D OBV values.
    """
    df['AD_OBV'] = 0.0  # Initialize AD OBV with float

    for i in range(1, len(df)):
        high_low_range = df.loc[i, 'high'] - df.loc[i, 'low']
        
        # Prevent division by zero for high-low range
        if high_low_range == 0:
            ad_multiplier = 0
        else:
            ad_multiplier = ((df.loc[i, 'close'] - df.loc[i, 'low']) / high_low_range)
        
        # Calculate A/D OBV
        df.loc[i, 'AD_OBV'] = df.loc[i-1, 'AD_OBV'] + (ad_multiplier * df.loc[i, 'volume'])
...
if __name__ == "__main__":
    ...
    for share in shares:
        ...
        # Calculate A/D OBV for each stock
        df['AD_OBV'] = calculate_ad_obv(df)
    ...
    # Display the combined DataFrame with A/D OBV
    print("Combined DataFrame with A/D OBV (combined_df):")
    print(combined_df[['symbol', 'close', 'AD_OBV']].head())  # Show the first few rows with A/D OBV

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_ad_obv.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'AD_OBV']].head())  # Show the first few rows of individual stock data with A/D OBV
