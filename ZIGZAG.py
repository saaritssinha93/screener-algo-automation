...
# Function to calculate ZigZag Indicator
def calculate_zigzag(df, pct_threshold=5):
    """
    Calculate the ZigZag Indicator.

    Parameters:
    df (pd.DataFrame): The DataFrame containing stock data with 'high' and 'low' columns.
    pct_threshold (float): The minimum percentage change to be considered a zigzag movement (default 5%).

    Returns:
    pd.DataFrame: A DataFrame with ZigZag points.
    """
    # Initialize the ZigZag column
    df['ZigZag'] = None

    # Initialize direction and last extreme point
    last_extreme_idx = 0
    direction = 0  # 0 = undefined, 1 = up, -1 = down

    for i in range(1, len(df)):
        # Calculate percentage change from the last extreme point
        high_pct_change = (df.loc[i, 'high'] - df.loc[last_extreme_idx, 'low']) / df.loc[last_extreme_idx, 'low'] * 100
        low_pct_change = (df.loc[last_extreme_idx, 'high'] - df.loc[i, 'low']) / df.loc[last_extreme_idx, 'high'] * 100

        # Determine if a new extreme point is reached
        if direction == 0:
            # Initial direction
            if high_pct_change > pct_threshold:
                direction = 1
                df.loc[i, 'ZigZag'] = df.loc[i, 'high']
                last_extreme_idx = i
            elif low_pct_change > pct_threshold:
                direction = -1
                df.loc[i, 'ZigZag'] = df.loc[i, 'low']
                last_extreme_idx = i
        elif direction == 1:
            # Current direction is up
            if low_pct_change > pct_threshold:
                direction = -1
                df.loc[i, 'ZigZag'] = df.loc[i, 'low']
                last_extreme_idx = i
            elif df.loc[i, 'high'] > df.loc[last_extreme_idx, 'high']:
                last_extreme_idx = i
                df.loc[i, 'ZigZag'] = df.loc[i, 'high']
        elif direction == -1:
            ...
            if high_pct_change > pct_threshold:
                ...
                df.loc[i, 'ZigZag'] = df.loc[i, 'high']
                last_extreme_idx = i
            elif df.loc[i, 'low'] < df.loc[last_extreme_idx, 'low']:
                last_extreme_idx = i
                df.loc[i, 'ZigZag'] = df.loc[i, 'low']

    # Forward fill and backward fill to remove NaN values in ZigZag column
    df.ffill(inplace=True)
    df.bfill(inplace=True)
    
    # Ensure downcasting of object dtype is handled by inferring object types
    df = df.infer_objects(copy=False)

    return df[['ZigZag']]

# Function to fetch historical price data and calculate ZigZag Indicator
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates ZigZag Indicator.
    ...
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data, and ZigZag points.
    """
    try:
        ...
        logging.info(f"Historical data fetched with ZigZag Indicator calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with ZigZag Indicator
    print(combined_df[['symbol', 'close', 'ZigZag']].head())  # Show key columns

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_ZigZag.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'ZigZag']].head())  # Show the first few rows with ZigZag points
