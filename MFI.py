...
# Function to fetch historical price data and calculate the Money Flow Index (MFI)
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day', mfi_window=14):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates the Money Flow Index (MFI).
    ...
    mfi_window (int): Period window for MFI calculation (default is 14).
    
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data, and MFI.
    """
    try:
        ...
        # Step 4: Calculate the Money Flow Ratio and MFI
        df['money_flow_ratio'] = df['money_flow_positive'].rolling(window=mfi_window).sum() / df['money_flow_negative'].rolling(window=mfi_window).sum()
        ...
        logging.info(f"Historical data fetched and MFI calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with MFI
    print(combined_df[['symbol', 'close', 'MFI']].head())  # Show the first few rows with MFI

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_mfi.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'MFI']].head())  # Show the first few rows of individual stock data with MFI
