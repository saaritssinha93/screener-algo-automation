...
# Function to calculate Price Oscillator (PPO)
def calculate_ppo(df, fast_period=12, slow_period=26):
    """
    Calculate the Price Oscillator (PPO).
    ...
    Returns:
    pd.Series: Price Oscillator (PPO) values.
    """
...
# Function to fetch historical price data and calculate PPO
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day', fast_period=12, slow_period=26):
    """
    Fetches historical price data and calculates Price Oscillator (PPO).
    ...
    fast_period (int): Period for the fast EMA in PPO calculation (default 12).
    slow_period (int): Period for the slow EMA in PPO calculation (default 26).
    
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data, and PPO.
    """
    try:
        ...
        df['PPO'] = calculate_ppo(df, fast_period, slow_period)

        # Handle NaN values using ffill and bfill
        df.ffill(inplace=True)
        df.bfill(inplace=True)

        logging.info(f"Historical data fetched and PPO calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with PPO
    print(combined_df[['symbol', 'close', 'PPO']].head())  # Show key columns with PPO

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_ppo.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'PPO']].head())  # Show the first few rows of individual stock data with PPO
