...
# Function to calculate the Elder-Ray Index (Bull Power and Bear Power)
def calculate_elder_ray(df, ema_period=13):
    ...
    ema_period (int): Period for the Exponential Moving Average (default is 13).
    
    Returns:
    pd.DataFrame: A DataFrame with Bull Power and Bear Power added.
    """
    df = df.copy()
    ...
    # Calculate Bull Power (High price minus EMA)
    df['Bull_Power'] = df['high'] - df['EMA']

    # Calculate Bear Power (Low price minus EMA)
...
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    ...
    try:
        ...
        logging.info(f"Historical data fetched and Elder-Ray Index calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with Elder-Ray Index (Bull Power and Bear Power)
    print(combined_df[['symbol', 'close', 'Bull_Power', 'Bear_Power']].head())  # Show key columns

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_Elder_Ray.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'Bull_Power', 'Bear_Power']].head())  # Show the first few rows of individual stock data
