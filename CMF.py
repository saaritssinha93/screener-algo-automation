...
def calculate_cmf(df, period=20):
    ...
    Parameters:
    df (pd.DataFrame): DataFrame with historical data (must include 'high', 'low', 'close', 'volume').
    period (int): Period for CMF calculation (default 20).
    
    Returns:
    pd.Series: Chaikin Money Flow values.
    """
    ...
    # Calculate the Chaikin Money Flow (CMF) by summing the money flow volume over the period
...
# Function to fetch historical price data and calculate CMF
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    """
    Fetches historical price data and calculates Chaikin Money Flow (CMF).
...
if __name__ == "__main__":
    ...
    print(combined_df[['symbol', 'close', 'CMF']].head())  # Show key columns with CMF
