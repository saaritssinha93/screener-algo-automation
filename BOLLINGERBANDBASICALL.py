...
# Function to calculate Bollinger Bands and related indicators
def calculate_bollinger_bands(df, window=20, num_std_dev=2):
    """
    Calculate Bollinger Bands, Bollinger Band Width, %B Indicator, Bollinger Band Squeeze, 
    and Bollinger Bands with Fibonacci Ratios.
    ...
    Returns:
    pd.DataFrame: Updated DataFrame with Bollinger Bands and related indicators.
    """
    ...
    # Calculate %B Indicator (Percentage Bandwidth)
    df['%B'] = (df['close'] - df['BB_Lower']) / (df['BB_Upper'] - df['BB_Lower'])

    # Calculate Bollinger Band Squeeze (When Bollinger Bands are narrow, indicating low volatility)
    df['BB_Squeeze'] = np.where(df['BB_Width'] < df['BB_Width'].rolling(window=window).mean(), 'squeeze', 'normal')

    # Bollinger Bands with Fibonacci Ratios
    df['BB_Fib_0.618'] = df['BB_Middle'] + 0.618 * (df['BB_Upper'] - df['BB_Middle'])
    df['BB_Fib_0.382'] = df['BB_Middle'] + 0.382 * (df['BB_Upper'] - df['BB_Middle'])
    df['BB_Fib_1.618'] = df['BB_Middle'] + 1.618 * (df['BB_Upper'] - df['BB_Middle'])

    # Forward and Backward fill NaN values to prevent errors
    df.ffill(inplace=True)
    df.bfill(inplace=True)

    return df

# Function to fetch historical price data and calculate Bollinger Bands and related indicators
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates Bollinger Bands 
    ...
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data, and Bollinger Bands with related indicators.
    """
    try:
        ...
        logging.info(f"Historical data fetched and Bollinger Bands calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with Bollinger Bands and related indicators
    print(combined_df[['symbol', 'close', 'BB_Upper', 'BB_Lower', 'BB_Width', '%B', 'BB_Squeeze', 'BB_Fib_0.618', 'BB_Fib_0.382', 'BB_Fib_1.618']].head())  # Show the first few rows with Bollinger Bands and related indicators

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_bollinger_bands.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'BB_Upper', 'BB_Lower', 'BB_Width', '%B', 'BB_Squeeze', 'BB_Fib_0.618', 'BB_Fib_0.382', 'BB_Fib_1.618']].head())  # Show the first few rows of individual stock data with Bollinger Bands and related indicators
