...
def calculate_kama(df, n=10, pow1=2, pow2=30):
    ...
    n (int): The number of periods (default is 10).
    pow1 (int): The short EMA smoothing constant (default is 2).
    pow2 (int): The long EMA smoothing constant (default is 30).
    
    Returns:
    pd.Series: A Series representing the KAMA values.
    """
    # Calculate the Efficiency Ratio (ER)
    change = abs(df['close'].diff(n))
    volatility = df['close'].diff().abs().rolling(n).sum()
    ER = change / volatility

    # Define smoothing constants (SC)
    fast_alpha = 2 / (pow1 + 1)
    slow_alpha = 2 / (pow2 + 1)
    SC = (ER * (fast_alpha - slow_alpha) + slow_alpha) ** 2

    # Calculate KAMA
    KAMA = np.zeros(df['close'].size)
    KAMA[:n] = df['close'][:n]  # Initialize KAMA with the first 'n' close values
    for i in range(n, len(df)):
        KAMA[i] = KAMA[i - 1] + SC[i] * (df['close'][i] - KAMA[i - 1])
...
# Function to fetch historical price data and calculate KAMA and ATR
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day', kama_window=10, atr_window=14):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates KAMA and ATR indicators.
    ...
    kama_window (int): Window size for KAMA calculation (default 10).
    atr_window (int): Window size for ATR calculation (default 14).
    
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data, KAMA, and ATR.
    """
    try:
        ...
        # Calculate KAMA
        df['KAMA'] = calculate_kama(df, n=kama_window)
        
        # Calculate ATR
        df['ATR'] = calculate_atr(df, atr_window)

        logging.info(f"Historical data fetched with KAMA and ATR calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with KAMA and ATR
    print(combined_df[['symbol', 'close', 'KAMA', 'ATR']].head())  # Show key columns

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_KAMA_ATR.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'KAMA', 'ATR']].head())  # Show the first few rows with KAMA and ATR
