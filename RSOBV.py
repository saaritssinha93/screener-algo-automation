...
# Function to calculate RS OBV (Relative Strength OBV)
def calculate_rs_obv(stock_df, benchmark_df):
    """
    Calculate Relative Strength OBV (RS OBV) between a stock and a benchmark index.
    
    Parameters:
    stock_df (pd.DataFrame): DataFrame containing OBV values for the stock.
    benchmark_df (pd.DataFrame): DataFrame containing OBV values for the benchmark.
    
    Returns:
    pd.Series: RS OBV values (the ratio of stock OBV to benchmark OBV).
    """
    rs_obv = stock_df['OBV'] / benchmark_df['OBV']
    return rs_obv
...
if __name__ == "__main__":
    ...
    shares = ['HDFCBANK', 'TCS']
    benchmark = 'NIFTYBEES'  # Use NIFTYBEES ETF as a benchmark
    ...
    # Fetch the historical data for the benchmark index (NIFTY50 in this case)
    benchmark_df = fetch_historical_data(kite, benchmark, from_date, to_date)
    benchmark_df['OBV'] = calculate_obv(benchmark_df)
    ...
    for share in shares:
        ...
        # Calculate RS OBV for each stock compared to the benchmark
        df['RS_OBV'] = calculate_rs_obv(df, benchmark_df)
    ...
    # Display the combined DataFrame with OBV and RS OBV
    print("Combined DataFrame with OBV and RS OBV (combined_df):")
    print(combined_df[['symbol', 'close', 'OBV', 'RS_OBV']].head())  # Show the first few rows with OBV and RS OBV

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_rs_obv.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'OBV', 'RS_OBV']].head())  # Show the first few rows of individual stock data with OBV and RS OBV
