...
def calculate_chandelier_exit(df, atr_multiplier=3, window=22):
    """
    ...
    window (int): The window size for calculating the highest high and lowest low (default is 22).
    
    Returns:
    pd.DataFrame: Updated DataFrame with Chandelier Exit for long and short positions.
    """
    df['ATR'] = calculate_atr(df, window=window)

    # Initialize columns for the Chandelier Exit
    df['Chandelier_Exit_Long'] = np.nan
    df['Chandelier_Exit_Short'] = np.nan

    # Calculate the highest high and lowest low for the given window
    df['Highest_High'] = df['high'].rolling(window=window).max()
    df['Lowest_Low'] = df['low'].rolling(window=window).min()

    # Set stop values for long and short positions using .loc to avoid chained assignment
    for i in range(window, len(df)):
        # For long positions (stop below the highest high)
        df.loc[i, 'Chandelier_Exit_Long'] = df.loc[i, 'Highest_High'] - (atr_multiplier * df.loc[i, 'ATR'])
        
        # For short positions (stop above the lowest low)
        df.loc[i, 'Chandelier_Exit_Short'] = df.loc[i, 'Lowest_Low'] + (atr_multiplier * df.loc[i, 'ATR'])

    return df

# Function to fetch historical price data and calculate Chandelier Exit
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day'):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates Chandelier Exit.
    ...
    Returns:
    pd.DataFrame: A DataFrame with Chandelier Exit for long and short positions.
    """
    try:
        ...
        logging.info(f"Historical data fetched and Chandelier Exit calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with Chandelier Exit
    print(combined_df[['symbol', 'close', 'Chandelier_Exit_Long', 'Chandelier_Exit_Short']].head())  # Show the first few rows with Chandelier Exit

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_chandelier_exit.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'Chandelier_Exit_Long', 'Chandelier_Exit_Short']].head())  # Show the first few rows of individual stock data with Chandelier Exit
