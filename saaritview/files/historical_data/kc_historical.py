...
try:
    ...
    logging.info("NSE instruments data saved to NSE_Instruments.csv")
except Exception as e:
...
def instrument_lookup(instrument_df, symbol):
    """Looks up instrument token for a given symbol from instrument dump."""
    try:
...
def fetch_ohlc(ticker, interval, duration):
    """Extracts historical OHLC data for the given ticker."""
    try:
...
# Example usage: Fetch historical OHLC data for ITC
ohlc_data = fetch_ohlc("ITC", "5minute", 5)
...
instrumentLookup(instrument_df,"ITC")
