...
# Function to fetch historical price data and calculate Chaikin Money Flow (CMF)
def fetch_historical_data(kite, stock_symbol, from_date, to_date, interval='day', cmf_window=21):
    """
    Fetches historical price data for a stock using Zerodha KiteConnect API and calculates the Chaikin Money Flow (CMF).
    ...
    cmf_window (int): Period window for CMF calculation (default is 21).
    
    Returns:
    pd.DataFrame: A DataFrame with open, high, low, close, volume data and CMF.
    """
    try:
        ...
        # Calculate the Money Flow Multiplier and Money Flow Volume
        ...
        df['CMF'] = df['money_flow_volume'].rolling(window=cmf_window).sum() / df['volume'].rolling(window=cmf_window).sum()
        ...
        logging.info(f"Historical data fetched and CMF calculated for {stock_symbol} from {from_date} to {to_date}.")
...
if __name__ == "__main__":
    ...
    # Display the combined DataFrame with CMF
    print(combined_df[['symbol', 'close', 'CMF']].head())  # Show the first few rows with CMF

    # Save the combined data to a CSV file
    combined_df.to_csv("combined_historical_data_with_cmf.csv", index=False)

    # Display individual DataFrames for each stock
    for stock, df in stock_data.items():
        print(f"\nData for {stock.replace('_df', '')}:")
        print(df[['close', 'CMF']].head())  # Show the first few rows of individual stock data with CMF
